...
const sourceLabels: Record<string, string> = {
  hero: "Главная страница — верхний блок",
  services: "Главная страница — блок услуг",
  okna: "Страница «Окна»",
  potolki: "Страница «Натяжные потолки»",
...
export async function POST(request: Request) {
  ...
  const textLines = [`Новая заявка — ${site.name}`, "", `Телефон: ${phone}`];
  ...
  const htmlRows = [`<p><b>Телефон:</b> ${escapeHtml(phone)}</p>`];
